...
def plot_lick_raster_with_states(ax, df, fig):
    ...
    for i, row in df.iterrows():
        try:
            ...
            # --- STATES ---
            led_on_start = safe_float(row['STATE_first_side_led_START']) - t0
            led_on_end   = safe_float(row['first_trial_response_time'])   - t0
            drink_start  = safe_float(row['STATE_iti_START']) - t0
            drink_end    = safe_float(row['STATE_iti_END'])   - t0
...
def plot_psychometric_curves(df, axes):
    ...
    for i, date in enumerate(last_five_dates):
        ...
        for task in tasks_today:
            ...
            group['delay_cues'] = group['delay_cues'].astype(float)
            group['response_side'] = group['response_side'].apply(lambda x: 1 if x == 'right' else 0)

            delays = np.sort(group['delay_cues'].unique())
            right_choice_freq = []
            for delay in delays:
                mask_delay = group['delay_cues'] == delay
                n_trials = mask_delay.sum()
                n_right = (mask_delay & (group['response_side'] == 1)).sum()
                right_choice_freq.append(n_right / n_trials if n_trials > 0 else np.nan)

            # Fit probit
            try:
                pars, _ = curve_fit(probit, group['delay_cues'], group['response_side'], p0=[0, 1])
            ...
            ax.scatter(delays, right_choice_freq, color=task_colors[task], s=20, alpha=0.8)
