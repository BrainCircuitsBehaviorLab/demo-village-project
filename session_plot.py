...
class SessionPlot(SessionPlotBase):
    ...
    def create_plot(
        ...
        if task == "S0":
            ...
            return self.plot_S5(df, width, height)
    ...
    def plot_S1(self, df: pd.DataFrame, width: float = 10, height: float = 8) -> Figure:
       ...
       # --- PLOT 3: Accuracy on trial ---
        ...
        handles = [
            ...
            mpatches.Patch(color='#76B7D2', label='Left lick'),
            mpatches.Patch(color='#F28E2B', label='Right lick'),
        ]
    ...
    def plot_S5(self, df: pd.DataFrame, width: float = 10, height: float = 8) -> Figure:
        df = assign_ports(df)
        df = parse_S5_data(df)
        # change the font in figure
        plt.rcParams.update({'font.size': 6, 'font.family': 'monospace'})
        ...
        # --- PLOT 1: accuracy and p ---
        plot_rolling_accuracy_and_p(df, ax1) #TODO
        ...
        # --- PLOT 5: lick raster and state machine ---
        plot_lick_raster_with_states_S5(ax5, df, fig)
        
        # --- PLOT 6: PC P(Choose right) vs delay  --- #TODO
        plot_psychometric_curve(df, ax6)

        # --- PLOT 7: histigram iti duration in the session  ---
        # plot_iti_histogram(ax7, df)
        plot_delay_probabilities(ax7, df)
